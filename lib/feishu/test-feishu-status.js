 ...
 * 飞书状态消息测试脚本
 * 
 * 这个脚本用于测试飞书MCP服务器是否可以正常发送状态消息。
 * 它会发送一条简单的状态消息到飞书群组。
 */

import FeishuMessenger from './src/messenger.js';

console.log('🚀 飞书状态消息测试');
 ...
 * 发送测试状态消息
 */
async function sendTestStatusMessage() {
  console.log(`\n📢 发送测试状态消息到飞书群组...`);
  ...
  try {
    ...
    // 发送状态卡片
    console.log('\n📝 正在发送消息...');
    const sendResult = await messenger.sendInteractiveCard(RECEIVE_ID, statusCard);
...
async function main() {
  ...
  // 2. 发送测试状态消息
  const messageSent = await sendTestStatusMessage();
  ...
  if (messageSent) {
    ...
    console.log('   - 状态消息发送成功');
    console.log('   - 请检查飞书群组是否收到消息');
  } else {
    console.log('❌ 测试失败！');
    console.log('   - 飞书API连接正常');
    console.log('   - 状态消息发送失败');
